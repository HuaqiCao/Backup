...
class QZSApp(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        left_layout.addWidget(
            row2('a_actual (mm):', self.a_actual_edit, sbold))
        left_layout.addWidget(
            row2('d_actual (mm):', self.d_actual_edit, sbold))
    ...
    def _plot_ax1(self):
        ...
        ax.set_ylim(-6, 6)
        ax.set_xlim(-3, 3)
        ax.set_title('Dimensionless F & K',
                     fontsize=self.TITLE_FS, fontweight='bold')
    ...
    def _plot_ax3(self):
        ...
        Ta_th = np.array([self._transmissibility(
            mu1, mu3, O, 0.5, 0.15) for O in Om])
        Ta_act = np.array([self._transmissibility(
            0.0022, 0.00065, O, 0.5, 0.18) for O in Om])
        ...
        ax.set_title('Displacement Transmissibility',
                     fontsize=self.TITLE_FS, fontweight='bold')
        ax.legend(['Theory', 'Actual'], loc='upper right',
    ...
    def _plot_geometry(self, a_target, h1, h_target, h2_target):
        ...
        try:
            ...
            secax.tick_params(colors=[0.85, 0.33, 0.10],
    ...
    def _update_psd_plots(self):
        ...
        try:
            ...
            Ta = self._transmissibility_vec(
                mu1, mu3, f_psd, f0_sys, Ze_hat, zeta)
