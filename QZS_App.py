...
class QZSApp(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        bold = QFont('Arial', 10, QFont.Bold)
        sbold = QFont('Arial', 9, QFont.Bold)

        def spin(val, lo=-1e6, hi=1e6, dec=3, step=0.001, w=80):
            s = QDoubleSpinBox()
        ...
        def row2(lbl_text, sp_widget, font=None):
            w = QWidget(); h = QHBoxLayout(w)
            h.setContentsMargins(0, 0, 0, 0)
            lbl = QLabel(lbl_text)
            lbl.setFont(font or bold)
            lbl.setFixedWidth(115)
            h.addWidget(lbl); h.addWidget(sp_widget)
            return w

        # ── dimensionless params ─────────────────────────────────────────────
        self.delta_hat_edit = spin(0.5)
        self.a_hat_edit     = spin(0.755)
        self.alpha_edit     = spin(0.942)
        self.alpha1_edit    = spin(0.501)
        self.gamma_edit     = spin(2.143)
        self.a_target_edit  = spin(60.0, dec=1, step=1)
        self.tau_p_edit     = spin(70.0, dec=1, step=1)
        self.G_edit         = spin(75000.0, dec=0, step=1000)
        ...
        for lbl, sp, cb in [
            ...
            ('τ_p (MPa):',     self.tau_p_edit,     self._calc_full),
            ('G (MPa):',       self.G_edit,         self._calc_full),
        ]:
            sp.valueChanged.connect(cb)
            left_layout.addWidget(row2(lbl, sp, bold))

        # ── Design Springs button ────────────────────────────────────────────
        design_btn = QPushButton('Design Springs')
        design_btn.setFont(QFont('Arial', 11, QFont.Bold))
        design_btn.setStyleSheet('background:#3F9A6B;color:white;')
        design_btn.setFixedHeight(32)
        design_btn.clicked.connect(self._save_design_data)
        left_layout.addWidget(design_btn)

        # ── spring matrix ────────────────────────────────────────────────────
        hdr = QWidget(); hh = QHBoxLayout(hdr); hh.setContentsMargins(0, 0, 0, 0)
        for t, w in [('Type', 42), ('n', 30), ('d mm', 38), ('D mm', 38)]:
            l = QLabel(t); l.setFont(sbold); l.setFixedWidth(w); hh.addWidget(l)
        left_layout.addWidget(hdr)

        self.U_turns = spin(17, 1, 999, 0, 1, 30)
        self.U_wire  = spin(1.2, 0.1, 99, 1, 0.1, 38)
        self.U_cyl   = spin(14.4, 0.1, 999, 1, 0.1, 38)
        self.M_turns = spin(32, 1, 999, 0, 1, 30)
        self.M_wire  = spin(1.2, 0.1, 99, 1, 0.1, 38)
        self.M_cyl   = spin(14.4, 0.1, 999, 1, 0.1, 38)
        self.D_turns = spin(17, 1, 999, 0, 1, 30)
        self.D_wire  = spin(1.2, 0.1, 99, 1, 0.1, 38)
        self.D_cyl   = spin(14.4, 0.1, 999, 1, 0.1, 38)
        self.B_turns = spin(16, 1, 999, 0, 1, 30)
        self.B_wire  = spin(1.2, 0.1, 99, 1, 0.1, 38)
        self.B_cyl   = spin(14.4, 0.1, 999, 1, 0.1, 38)
        ...
        for label, s_n, s_d, s_D, n_k, d_k, D_k in [
            ...
            lb = QLabel(label); lb.setFont(sbold); lb.setFixedWidth(42); rl.addWidget(lb)
        ...
        # ── geometry rows (W / H / D) ────────────────────────────────────────
        ghdr = QWidget(); gh = QHBoxLayout(ghdr); gh.setContentsMargins(0, 0, 0, 0)
        for t, w in [('', 42), ('W', 30), ('H', 38), ('D', 38)]:
            l = QLabel(t); l.setFont(sbold); l.setFixedWidth(w); gh.addWidget(l)
        ...
        def geom_row(label, wv, hv, dv):
            ...
            lb = QLabel(label); lb.setFont(sbold); lb.setFixedWidth(42); rl.addWidget(lb)
            spins = []
            for val, width in [(wv, 30), (hv, 38), (dv, 38)]:
                s = QDoubleSpinBox()
                s.setDecimals(0); s.setRange(-9999, 9999)
                s.setValue(val); s.setFixedWidth(width)
        ...
        self.h4_edit      = spin(self.h4, dec=0, step=1)
        self.a_actual_edit = spin(60.0, dec=0, step=1)
        self.d_actual_edit = spin(self.d_actual, dec=0, step=1)
        ...
        left_layout.addWidget(row2('h4 (mm):', self.h4_edit, sbold))
        left_layout.addWidget(row2('a_actual (mm):', self.a_actual_edit, sbold))
        left_layout.addWidget(row2('d_actual (mm):', self.d_actual_edit, sbold))

        # ── displacement preview ─────────────────────────────────────────────
        sep = QFrame(); sep.setFrameShape(QFrame.HLine)
        sep.setStyleSheet('color: #aaa;'); left_layout.addWidget(sep)
        ...
        left_layout.addWidget(row2('y preview (mm):', self.y_disp_edit, sbold))

        # ── Load CSV ─────────────────────────────────────────────────────────
        load_btn = QPushButton('Load & Get PSD')
        load_btn.setFont(QFont('Arial', 11, QFont.Bold))
        load_btn.setStyleSheet('background:#E07320;color:white;')
        load_btn.setFixedHeight(32)
        load_btn.clicked.connect(self._process_vibration_signals)
        left_layout.addWidget(load_btn)

        self.C_edit  = spin(20.0, dec=1, step=0.5)
        self.Ze_edit = spin(3.0,  dec=1, step=0.1)
        self.C_edit.valueChanged.connect(self._calc_full)
        self.Ze_edit.valueChanged.connect(self._calc_full)
        left_layout.addWidget(row2('Damping C:', self.C_edit, sbold))
        left_layout.addWidget(row2('Ze (mm):', self.Ze_edit, sbold))
    ...
    def _f_actual_N(self, y_arr, a, d):
        """Force [N] vs displacement y [mm] — formula from f_y.py.
        Free lengths: 119.2 mm (oblique springs), 153.1 mm (vertical).
        Spring rates:  0.362 N/mm (upper/lower), 0.1825 N/mm (mid), 0.3843 N/mm (bottom).
        a: horizontal arm [mm], d: vertical offset [mm].
        """
        def L(x, dv): return np.sqrt(np.maximum(x**2 + dv**2, 1e-12))
        L1 = L(a + y_arr, d); L2 = L(y_arr, d); L3 = L(a - y_arr, d)
        f  = (-2*(119.2 - L1)*(a + y_arr)*0.362  / L1 +
               2*(119.2 - L2)*(-y_arr)   *0.1825 / L2 +
               2*(119.2 - L3)*(a - y_arr)*0.362  / L3 +
              (153.1 - 67 + y_arr)*0.3843)
        return f
    ...
    def _calc_full(self, *_):
        ...
        d_act = self.d_actual_edit.value()
        self.f_actual_real = self._f_actual_N(self.y_dim, a_act, d_act)
    ...
    def _plot_ax1(self):
        ...
        ax.set_title('Dimensionless F & K', fontsize=self.TITLE_FS, fontweight='bold')
    ...
    def _plot_ax_dim(self):
        ...
        ax.tick_params(axis='y', labelcolor=[0.0,0.45,0.74], labelsize=self.TICK_FS)
        ax.set_title('Force & Stiffness', fontsize=self.TITLE_FS, fontweight='bold')
    ...
    def _plot_ax3(self):
        ...
        Ta_th  = np.array([self._transmissibility(mu1, mu3, O, 0.5, 0.15) for O in Om])
        Ta_act = np.array([self._transmissibility(0.0022, 0.00065, O, 0.5, 0.18) for O in Om])
        ...
        ax.set_xlabel('Frequency', fontsize=self.LABEL_FS)
        ax.set_ylabel('Transmissibility $T_a$', fontsize=self.LABEL_FS)
        ax.set_title('Displacement Transmissibility', fontsize=self.TITLE_FS, fontweight='bold')
        ax.legend(['Theory', 'Actual'], loc='upper right', fontsize=self.LEGEND_FS)
    ...
    def _plot_geometry(self, a_target, h1, h_target, h2_target):
        ...
        ax.set_title('QZS Assembly — Side View', fontsize=self.TITLE_FS,
                     fontweight='bold')
    ...
    def _update_psd_plots(self):
        ...
        try:
            ...
            ax4.plot(t_in[idx], v_out[idx], color=[0,0.447,0.741], lw=1.2,
                     label='Group1 Output')
            ax4.set_title('Time Domain (first 2 s)',
                          fontsize=self.TITLE_FS, fontweight='bold')
            ...
            ax5.set_title('Power Spectrum Density Comparison',
                          fontsize=self.TITLE_FS, fontweight='bold')
