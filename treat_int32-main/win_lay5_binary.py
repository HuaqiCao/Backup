...
class _matchedfilter(lay5.Ui_MainWindow):
    ...
    def _trigger_all(self):
        ...
        for k in range(5,tot_num):
            ...
            vt_corr = arr_x2 - np.mean(arr_x2)
            st_corr = st - np.mean(st)

            corr= np.sum(vt_corr * st_corr) / np.sqrt(np.sum(np.power(vt_corr,2)) * np.sum(np.power(st_corr,2)) )
