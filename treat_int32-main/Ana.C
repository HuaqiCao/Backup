...
void Ana(){
	...
	Double_t        bl_slope;
	Double_t        bl_chi2;
	TFile *f1 = new TFile("/Users/fangxie/CUPID/PulseShapeAnalysis/FDURUN2510/BINData/data/TriggerEvent.root","READ");
	...
	std::cout << "第一个事件的Amp_raw: " << Amp_filtered << std::endl;

	TFile *f2 = new TFile("result.root","RECREATE");
	...
	// 设置点的样式和颜色
	//g1->SetMarkerStyle(20);  // 点的样式
	g1->SetMarkerColor(kRed); // 颜色
	//g1->SetMarkerSize(0.8);   // 点的大小
	// 或者连成线
  ...
  hamp1->GetYaxis()->SetTitle("Counts");
	//hamp3->GetXaxis()->SetTitle("xxxx");
	//hamp4->GetXaxis()->SetTitle("Amplitude");
	//hamp4->GetXaxis()->SetTitle("xxxx");
