...
void Ana() {
    ...
    // Create output directory "Ana" in the same path as input file
    ...
    gROOT->SetBatch(kTRUE); // Run without pop-up windows

    // 2. Open Input File and Get Tree
    ...
    if (!T1) {
        std::cout << "TTree 'tree1' not found. Exit." << std::endl;
        f1->Close();
        return;
    }
    ...
    T1->GetEntry(0);
    std::cout << "Amp_filtered of first event: " << Amp_filtered << std::endl;

    // 4. Initialize Output ROOT File and Histograms
    ...
    TH2D *h2_RiseDecay = new TH2D("h2_RiseDecay",";RiseTime;DecayTime", 400, 0, 400, 400, 0, 400);
    h2_RiseDecay->SetDirectory(nullptr);

    TH2D *h2_RawVsFiltered = new TH2D("h2_RawVsFiltered",";Raw Amplitude;Filtered Amplitude", 40000, 0, 300000000, 40000, 0, 300000000);
    ...
    TGraph *g1 = new TGraph(); TGraph *g2 = new TGraph(); TGraph *g3 = new TGraph();
    TGraph *g4 = new TGraph(); TGraph *g5 = new TGraph(); TGraph *g6 = new TGraph();
    TGraph *g7 = new TGraph(); TGraph *g8 = new TGraph(); TGraph *g9 = new TGraph();

    int k1 = 0;

    // 5. Initialize Output Cut Tree
    ...
    // 8. Visualization and Save PNGs
    gStyle->SetOptStat(0);

    // 1) Baseline vs Amplitude
    TCanvas *cBaselineVsAmplitude = new TCanvas("cBaselineVsAmplitude","Baseline vs Amplitude",0, 0, 800, 600);
    ...
    f2->cd(); cBaselineVsAmplitude->Write();

    // 2) Time vs Baseline
    TCanvas *cTimeVsBaseline = new TCanvas("cTimeVsBaseline","Time vs Baseline",0, 0, 800, 600);
    cTimeVsBaseline->cd();
    ...
    f2->cd(); cTimeVsBaseline->Write();

    // 3) RiseTime vs DecayTime
    TCanvas *cRiseTimeVsDecayTime = new TCanvas("cRiseTimeVsDecayTime","RiseTime vs DecayTime",0, 0, 800, 600);
    ...
    f2->cd(); cRiseTimeVsDecayTime->Write();

    TCanvas *cRiseDecay2D = new TCanvas("cRiseDecay2D","RiseTime vs DecayTime (2D)",0, 0, 800, 600);
    cRiseDecay2D->cd();
    h2_RiseDecay->Draw("COLZ");
    cRiseDecay2D->SaveAs((outDir + "/cRiseDecay2D.png").Data());
    f2->cd(); cRiseDecay2D->Write();

    // 4) Chi2 Fit Comparison
    TCanvas *cLstsqRawfitVsLstsqFilterfit = new TCanvas("cLstsqRawfitVsLstsqFilterfit","Lstsq_rawfit vs Lstsq_filterfit",0, 0, 800, 600);
    cLstsqRawfitVsLstsqFilterfit->cd();
    g4->SetMarkerColor(kRed);
    g4->Draw("AP");
    g4->GetXaxis()->SetTitle("chi2_rawfit");
    g4->GetYaxis()->SetTitle("chi2_filterfit");
    cLstsqRawfitVsLstsqFilterfit->SaveAs((outDir + "/cLstsqRawfitVsLstsqFilterfit.png").Data());
    f2->cd(); cLstsqRawfitVsLstsqFilterfit->Write();

    // 5) Amplitude Fit Comparison
    TCanvas *cAmpRawfitVsAmpFilterfit = new TCanvas("cAmpRawfitVsAmpFilterfit","Amp_rawfit vs Amp_filterfit",0, 0, 800, 600);
    cAmpRawfitVsAmpFilterfit->cd();
    g5->SetMarkerColor(kRed);
    g5->Draw("AP");
    g5->GetXaxis()->SetTitle("Amp_rawfit");
    g5->GetYaxis()->SetTitle("Amp_filterfit");
    cAmpRawfitVsAmpFilterfit->SaveAs((outDir + "/cAmpRawfitVsAmpFilterfit.png").Data());
    f2->cd(); cAmpRawfitVsAmpFilterfit->Write();

    // 6) Chi2 vs TVL
    TCanvas *cChi2filteredVsTVL = new TCanvas("cChi2filteredVsTVL","Chi2filtered vs TVL",0, 0, 800, 600);
    ...
    f2->cd(); cChi2filteredVsTVL->Write();

    // 7) Chi2 vs TVR
    TCanvas *cChi2filteredVsTVR = new TCanvas("cChi2filteredVsTVR","Chi2filtered vs TVR",0, 0, 800, 600);
    ...
    f2->cd(); cChi2filteredVsTVR->Write();

    // 8) Amp vs Chi2
    TCanvas *cAmplitudeVsChi2filtered = new TCanvas("cAmplitudeVsChi2filtered","Amplitude vs Chi2filtered",0, 0, 800, 600);
    cAmplitudeVsChi2filtered->cd();
    g8->SetMarkerColor(kRed);
    g8->Draw("AP");
    g8->GetXaxis()->SetTitle("Amplitude");
    g8->GetYaxis()->SetTitle("Chi2Filtered");
    cAmplitudeVsChi2filtered->SaveAs((outDir + "/cAmplitudeVsChi2filtered.png").Data());
    f2->cd(); cAmplitudeVsChi2filtered->Write();

    // 9) Amp vs Decay Time
    TCanvas *cAmplitudeVsDecayTime = new TCanvas("cAmplitudeVsDecayTime","Amplitude vs Decay Time",0, 0, 800, 600);
    cAmplitudeVsDecayTime->cd();
    g9->SetMarkerColor(kRed);
    g9->Draw("AP");
    g9->GetXaxis()->SetTitle("Amplitude");
    g9->GetYaxis()->SetTitle("Decay Time");
    cAmplitudeVsDecayTime->SaveAs((outDir + "/cAmplitudeVsDecayTime.png").Data());
    f2->cd(); cAmplitudeVsDecayTime->Write();

    // 10) Amplitude Spectra Comparison
    TCanvas *cAmplitudeSpectrum = new TCanvas("cAmplitudeSpectrum","Amplitude Spectrum",0, 0, 800, 600);
    ...
    leg1->Draw();
    cAmplitudeSpectrum->SaveAs((outDir + "/cAmplitudeSpectrum.png").Data());

    // 11) Raw vs Filtered Amp (2D)
    TCanvas *cRawVsFiltered2D = new TCanvas("cRawVsFiltered2D","Raw vs Filtered Amplitude",0, 0, 800, 600);
    cRawVsFiltered2D->cd();
    h2_RawVsFiltered->Draw("COLZ");
    cRawVsFiltered2D->SaveAs((outDir + "/cRawVsFiltered2D.png").Data());
    f2->cd(); cRawVsFiltered2D->Write();

    // 9. Save and Close All
    f2->cd();
    hamp1->Write(); hamp2->Write(); hamp3->Write();
    hamp4->Write(); hamp5->Write();
    h2_RiseDecay->Write(); h2_RawVsFiltered->Write();
    f2->Close();

    f1->Close();
    std::cout << "Analysis complete. Results in: " << outDir << std::endl;
}