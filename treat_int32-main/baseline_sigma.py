...
def __run():
    ...
    fname, ftype = QFileDialog.getOpenFileName(None, "Open File", "/home/duandy/disk/bolometer/Data/RUN33", "")
    if len(fname) == 0:
    ...
    while line:
        ...
        x = []
        num = 0
        opfile.seek(2 * (int(filedata[0])) - wl, 0)
    ...
    xn = np.arange(low_limit, low_limit + bin_num * bin_width, bin_width)
    fig, ax = plt.subplots(nrows=1, ncols=1)
    arr_yhist_diff_filtered_2t = plt.hist(arr_y_diff_filtered_2t, bins=bin_num, range=(low_limit, up_limit),
                                          histtype='bar')
    # Executing curve_fit on noisy data
    popt4, pcov4 = curve_fit(__func, xn, arr_yhist_diff_filtered_2t[0])
    ...
    ax.set_title(r'Histogram of Baseline: $\sigma$={0:.2f} ADC'.format(abs(popt4[2])))
    ...
    # Executing curve_fit on noisy data
    popt, pcov = curve_fit(__func, xn_raw, arr_yhist_raw[0])
    # popt returns the best fit values for parameters of the given model (func)
    print(abs(popt))
    ...
    ax2.legend()
    ax2.grid(True)
    ax2.set_xlabel('Baseline (ADC)')
    ax2.set_ylabel('Counts')
    ax2.set_title(r'Histogram of Baseline: $\sigma$={0:.2f} ADC'.format(abs(popt[2])))
