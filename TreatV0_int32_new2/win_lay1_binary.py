...
class _pulse_inspect(lay1.Ui_MainWindow):
    ...
    def _browserfile(self):
        ...
        fname,ftype = QFileDialog.getOpenFileName(self, "Open File", "/mnt/d/RUNs_data_analysis/RUN2410/Converted_Data/tdms/BKG_RUN2410_2_2")#, "root file(*.root)")#如果添加一个内容则需要加两个分号
        if len(fname) == 0:
            print("fname")
            return
        print(fname)
