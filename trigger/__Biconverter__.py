...
def run():
    ...
    data_light = all_group_channels[1]
    ...
    converted_lightt = np.zeros(20000)
    for num in range(0, tot_num):
        if num%100 == 0:
            print(num//100)
        #converted_heat = np.int32((data_heat[20000*num:20000*(num+1)]+5.0)*(2**15)/5)
        #converted_light = np.int32((data_light[20000*num:20000*(num+1)]+5.0)*(2**15)/5)
        converted_heat = ((data_heat[20000*num:20000*(num+1)]+5.0)*(2**15)/5).astype(np.dtype('<u4'))
        converted_light = ((data_light[20000*num:20000*(num+1)]+5.0)*(2**15)/5).astype(np.dtype('<u4'))
