% input voltage data from multiple CSV files
% plot PSD & LPSD & RMS

function PSD_LPSD_RMS_no()
%% === 1. 文件选择 ===
[fileNames, filePath] = uigetfile('*.csv', '选择voltage信号 CSV 文件', 'MultiSelect', 'on');
...
for iFile = 1:numFiles
    ...
    time = data(:,1);
    voltage = data(:,2);  % 读取电压值
    
    % 转换为加速度值
    a_base = voltage / (sensitivity * gain);
    ...
    % LPSD 计算
    lpsd_acc = sqrt(pxx);                   % [g/√Hz] (已包含灵敏度转换)
    lpsd_disp = g ./ ((2*pi*f).^2) .* lpsd_acc; % [m/√Hz]
    
    %% RMS 计算
    RMS_result = zeros(size(band_edges,1), 2);
    for iBand = 1:size(band_edges,1)
        idx = f >= band_edges(iBand,1) & f <= band_edges(iBand,2);
        RMS_result(iBand, 1) = sqrt(sum((lpsd_acc(idx).^2) * df));     % 加速度 [g]
        RMS_result(iBand, 2) = sqrt(sum((lpsd_disp(idx).^2) * df));    % 位移 [m]
    end
    allRMS{iFile} = RMS_result;
    
    %% 绘制图形
    styleID = mod(iFile-1, numel(markers)) + 1;
    plot(ax1, f, pxx, 'Color', colors(iFile,:), 'LineStyle', markers{styleID}, 'LineWidth', 1.5, 'DisplayName', nameOnly);
