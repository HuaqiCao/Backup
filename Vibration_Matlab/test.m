function spring_design_analysis_main()
%% === 1. 文件选择 ===
...
sensitivity = 1.026; % 默认g/V为1.026
g = 9.81;
lpsd_acc = sqrt(pxx);                 % 单位 g/sqrt(Hz)
lpsd_vel = g ./ (2*pi*f) .* lpsd_acc; % 单位 m/s/sqrt(Hz)
lpsd_disp = g ./ ((2*pi*f).^2) .* lpsd_acc; % 单位 m/sqrt(Hz)
...
for i = 1:size(band_edges,1)
    ...
    RMS_result(i, :) = [rms_acc, rms_disp*1e9]; % 输出位移单位：nm
end
...
loglog(f, source_lpsd_acc, 'b-', 'LineWidth', 1.5, 'DisplayName', '振源'); hold on;
if ~isempty(best_H)
...
loglog(f, source_lpsd_disp*1e9, 'b-', 'LineWidth', 1.5, 'DisplayName', '振源'); hold on;
if ~isempty(best_H)
