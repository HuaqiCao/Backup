...
for i = 1:length(d_wire_range)
    ...
    figSize = [1, 1, 10, 5];  % in inches
        % === Figure 1: Frequency Response Curve (Modified) ===
    ...
    grid on;
    set(gca, 'FontName', 'Arial', 'FontSize', 12); % Render font for Figure 3
        % ============== 减振效果分析 ==============
    % 选择 CSV 文件读取加速度数据
    ...
    nfft = 500000; % Modified: set to 500000
    window = hanning(nfft);
    ...
    loglog(f, pxx_base / g^2, 'b-', 'LineWidth', 1.5, 'DisplayName', 'Before Isolation on MXC_vertical @RT');
    hold on;
    loglog(f, pxx_isolated / g^2, 'r-', 'LineWidth', 1.5, 'DisplayName', 'After Isolation on MXC simulation_vertical @RT');
    xlabel('Frequency (Hz)');
    ylabel('PSD [g²/Hz]');
    title('Power Spectral Density Comparison (Vertical)', 'FontSize', 24);
    legend('show', 'Location', 'best', 'FontSize', 16);
    grid on;
    xlim([0.1, 200]);

    % === LPSD Acceleration (Modified) ===
    fig_acc_lpsd = figure('Name', 'LPSD of Acceleration', 'Units', 'inches', 'Position', figSize);
    loglog(f, lpsd_base, 'b-', 'LineWidth', 1.5, 'DisplayName', 'Before Isolation on MXC_vertical @RT');
    hold on;
    loglog(f, lpsd_isolated, 'r-', 'LineWidth', 1.5, 'DisplayName', 'After Isolation on MXC simulation_vertical @RT');
    xlabel('Frequency (Hz)');
    ylabel('LPSD [g/√Hz]');
    title('Linear Power Spectral Density of Acceleration (Vertical)', 'FontSize', 24);
    legend('show', 'Location', 'best', 'FontSize', 16);
    grid on;
    xlim([0.1, 200]);

    % === LPSD Displacement (Modified) ===
    fig_disp_lpsd = figure('Name', 'LPSD of Displacement', 'Units', 'inches', 'Position', figSize);
    loglog(f, lpsd_base_disp * 1e9, 'b-', 'LineWidth', 1.5, 'DisplayName', 'Before Isolation on MXC_vertical @RT');
    hold on;
    loglog(f, lpsd_isolated_disp * 1e9, 'r-', 'LineWidth', 1.5, 'DisplayName', 'After Isolation on MXC simulation_vertical @RT');
    xlabel('Frequency (Hz)');
    ylabel('LPSD [nm/√Hz]');
    title('Linear Power Spectral Density of Displacement (Vertical)', 'FontSize', 24);
    legend('show', 'Location', 'best', 'FontSize', 16);
    grid on;
    xlim([0.1, 200]);
        % === RMS Calculation ===
    ...
    % === Create Table Data (Modified) ===
    rms_results = cell(nBands, 7);
    for iBand = 1:nBands
        rms_results{iBand,1} = band_names{iBand};
        rms_results{iBand,2} = sprintf('%.2f', RMS_base_acc(iBand));     % Format as fixed decimal
        rms_results{iBand,3} = sprintf('%.2f', RMS_isolated_acc(iBand)); % Format as fixed decimal
        rms_results{iBand,4} = sprintf('%.2f', reduction_acc(iBand));    % Format as fixed decimal
        rms_results{iBand,5} = sprintf('%.2f', RMS_base_disp(iBand));    % Format as fixed decimal
        rms_results{iBand,6} = sprintf('%.2f', RMS_isolated_disp(iBand));% Format as fixed decimal
        rms_results{iBand,7} = sprintf('%.2f', reduction_disp(iBand));   % Format as fixed decimal
    end
    ...
    % MATLAB uitable with larger header font
    t = uitable(fig_rms, ...
            'Data', rms_results(display_idx, :), ...
            'ColumnName', col_names, ...
            'Units', 'normalized', ...
            'Position', [0.05, 0.05, 0.9, 0.9], ...
            'FontSize', 10, ...  % Table content font size
            'RowName', []);
