...
fprintf('预压缩量 上：delta_target=%.1fmm, 中：delta1_target=%.1fmm, 下：delta2_target=%.1fmm, 底部（压簧）：delta3_target =%.1fmm\n\n',delta_target,delta1_target,delta2_target,delta3_target);
...
%M_limit=((delta3_target/1000)*k2)/g; %k2
%M1_limit=((delta2_target/1000)*k1)/g; %up and down
%M2_limit=((delta1_target/1000)*k3)/g; %middle
%fprintf('初始时：M_limit=%.1f,M1_limit=%.1f,M2_limit=%.1f\n',M_limit,M1_limit,M2_limit);
...
%M_limit=((delta3_eq/1000)*k2)/g; %k2
%M1_limit=((delta_eq/1000)*k1)/g; %up and down
%M2_limit=((delta1_eq/1000)*k3)/g; %middle
%fprintf('平衡时：M_limit=%.1f,M1_limit=%.1f,M2_limit=%.1f\n',M_limit,M1_limit,M2_limit);
...
excel_filename = fullfile(path, 'Spring_Parameters_0.5ratio_2kg_25mm_304.xlsx');

%%如果K2变成拉簧
%%162mm为MXC上方的距离
L_k2 = 162-delta3_eq-69.4; %此时K2的原长+弹簧的伸长量 %30mm security length
fprintf('此时K2弹簧的原长+弹簧的原长（拉簧）：L_k2=%0.1fmm\n',L_k2);

% 存储K2弹簧参数——拉簧
k2_results = [];
for C = C_range
    D = sqrt(((k2/1000)*8*(C^4)*L_k2)/(G*ratio));
    d = D/C; %K2
    D_out = D+d; %弹簧外径
    ...
    p = ratio * D;
    n_actual = L_k2/p + 2; % actual turns %pluse 2
    %fprintf('K2:n=%0.1f,n_actual=%0.1f\n',n,n_actual) %comfrim turns
    k2_results = [k2_results; d,D,D_out,C,n_actual,ratio,p,G,L_k2,k2_actual*1000,tau,tau_p];
    end
end

if ~isempty(k2_results)
    k2_table = array2table(k2_results, 'VariableNames', {'d_mm', 'D_mm','D_out_mm','C', 'n_actual', 'ratio', 'p_mm', 'G_Mpa', 'L_mm', 'k_actual_N_m','Tau','Tau_p'});
    [~, ia] = unique(k2_table(:, {'C', 'ratio'}), 'rows');
    k2_table = k2_table(ia, :);
    writetable(k2_table, excel_filename, 'Sheet', 'K2_Spring');
else
    warning('K2_Spring：没有任何参数满足应力条件，跳过写入。');
end

%% 上&下弹簧
k1_results = [];
for C1 = C1_range
    D1 = sqrt(((k1/1000)*8*(C1^4)*L1)/(G*ratio1));
    d1 = D1/C1; %K2
    D_out1 = D1 + d1; %弹簧外径
    ...
    p1 = ratio1 * D1;
    n1_actual = L1/p1 + 2; % actual turns %pluse 2
    %fprintf('K1:n=%0.1f,n_actual=%0.1f\n',n1,n1_actual) %comfrim turns
    k1_results = [k1_results; d1,D1,D_out1,C1,n1_actual,ratio1,p1,G,L1,k1_actual*1000,tau1,tau_p];
    end
end

if ~isempty(k1_results)
    k1_table = array2table(k1_results, 'VariableNames', {'d_mm', 'D_mm','D_out_mm', 'C', 'n_actual', 'ratio', 'p_mm', 'G_Mpa', 'L_mm', 'k_actual_N_m','Tau','Tau_p'});
    [~, ia] = unique(k1_table(:, {'C', 'ratio'}), 'rows');
    k1_table = k1_table(ia, :);
    writetable(k1_table, excel_filename, 'Sheet', 'Up_Down_Spring');
else
    warning('Up_Down_Spring：没有任何参数满足应力条件，跳过写入。');
end

%% 存储中间弹簧参数（侧边）
k3_results = [];
for C2 = C2_range
    D2 = sqrt(((k3/1000)*8*(C2^4)*L2)/(G*ratio2));
    d2 = D2/C2; %K2
    D_out2 = D2 + d2;
    ...
    p2 = ratio2 * D2;
    n2_actual = L2/p2 + 2; % actual turns %pluse 2
    %fprintf('K3:n=%0.1f,n_actual=%0.1f\n',n2,n2_actual) %comfrim turns
    ...
    k3_results = [k3_results; d2,D2,D_out2,C2,n2_actual,ratio2,p2,G,L2,k3_actual*1000,tau2,tau_p];
    end
end

if ~isempty(k3_results)
    k3_table = array2table(k3_results, 'VariableNames', {'d_mm', 'D_mm','D_out_mm', 'C', 'n_actual', 'ratio', 'p_mm', 'G_Mpa', 'L_mm', 'k_actual_N_m','Tau','Tau_p'});
    [~, ia] = unique(k3_table(:, {'C', 'ratio'}), 'rows');
    k3_table = k3_table(ia, :);
    writetable(k3_table, excel_filename, 'Sheet', 'Middle_Spring');
else
    warning('Middle_Spring：没有任何参数满足应力条件，跳过写入。');
end
