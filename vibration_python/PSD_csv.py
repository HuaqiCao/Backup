...
def process_files():
    ...
    for idx, file_path in enumerate(file_paths):
        ...
        try:
            ...
            if ext == '.txt':
                ...
                # Process CSV file
                df = pd.read_csv(file_path, header=None, dtype={0: 'str', 1: 'str'})
            ...
            # Record peak data (frequency, peak value) without filename
            for peak in peaks:
                peak_freq = freqs[peak]
                peak_value = lpsd[peak]
                peak_data.append([peak_freq, peak_value])  # Remove filename
    ...
    # Sort the peaks by value, in descending order, and select the top 20
    top_peaks = peak_df.sort_values(by="Peak Value", ascending=False).head(20)

    # Add sequential numbering (1, 2, 3, 4, ...) to the output DataFrame
    top_peaks.reset_index(drop=True, inplace=True)  # Reset index to start from 0
    top_peaks.index += 1  # Add 1 to start the index from 1

    # Print the result with sequential numbering
    print("\nTop 20 Peaks (sorted by peak value):")
    print(top_peaks)
